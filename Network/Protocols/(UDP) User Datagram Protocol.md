 UDP is a simple connectionless protocol that operates at the [[(OSI) Open Systems Interconnection-Modell#4. Transportschicht (Transport, Layer 4)|transport layer (Layer 4)]] of the OSI model. Being connectionless means that it does not need to establish a connection. UDP does not even provide a mechanism to know that the packet has been delivered.
 Because of the smaller overhead and smaller and simpler header UDP is able to send data faster than TCP
Uses:
